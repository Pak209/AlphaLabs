...
// Data source: live is the DEFAULT. Screen A is driven by a single read-only GET
// to /api/review/briefing; Screen B (Opportunity Detail) is driven by a read-only
// GET /api/review/opportunity/{idea_id} fetched on tap. window.REVIEW_MOCK is the
// review.v1 fallback dataset. Mock is an explicit dev override via a query flag
// (?review_data=mock or #/review?data=mock), and Screen A also falls back to mock
// automatically whenever the live briefing fetch or schema validation fails (a
// visible warning is shown). Screen B never falls back to mock in live mode: a
// failed/absent detail shows an honest loading / not-found / unavailable state.
// Nothing mutates. Decision buttons remain inert: they console.log the endpoint
// they WOULD call and show a local toast, but never POST or hit the network.
// Screen C (approval queue) has no live wiring and shows an honest placeholder.
(function () {
  ...
  const state = {
    ...
    // Screen B live wiring (read-only). In live mode Screen B fetches
    // GET /api/review/opportunity/{idea_id}; these track that request without
    // ever falling back to mock detail while in live mode.
    detailLoading: false,               // live fetch in flight
    detailError: "",                    // "" | "not_found" | human-readable warning
    liveDetail: null,                   // validated review.v1 detail for currentOppId
  };

  /* ---------------- small helpers ---------------- */
  const esc = (s) => String(s).replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c]));
  // Escaped value, or an em-dash when null/undefined/empty (honest "no value").
  const orDash = (v) => (v === null || v === undefined || v === "") ? "—" : esc(v);

  // Resolve the detail object for an idea_id. In mock mode this is the mock
  // dataset; in live mode it is the validated payload already fetched for the
  // current opportunity (never the mock — live mode must not show mock detail).
  function resolveDetail(id) {
    if (dataMode === "live") {
      const d = state.liveDetail;
      if (d && d.header && Number(d.header.idea_id) === Number(id)) return d;
      return null;
    }
    return R.get_opportunity(id);
  }
  ...
  // Structural validation for an OpportunityDetail payload. Returns "" when the
  // payload is review.v1 and carries the sections Screen B renders; otherwise a
  // human-readable reason. Sections are allowed to be empty/null (honest
  // unavailable states) — only the top-level shape is required here.
  function validateOpportunity(p) {
    if (!p || typeof p !== "object") return "response was not a JSON object";
    const m = p.meta;
    if (!m || typeof m !== "object") return "response is missing meta envelope";
    if (m.schema_version !== "review.v1") return "unexpected schema_version (need review.v1)";
    if (!m.data_freshness || !m.safety_status) return "meta is missing freshness/safety status";
    if (!p.header || typeof p.header !== "object") return "missing header";
    if (!p.conviction || typeof p.conviction !== "object") return "missing conviction";
    if (!p.thesis || typeof p.thesis !== "object") return "missing thesis";
    if (!Array.isArray(p.confidence_breakdown)) return "missing confidence_breakdown";
    if (!p.ai_explanation || typeof p.ai_explanation !== "object") return "missing ai_explanation";
    return "";
  }

  // Read-only GET for a single opportunity. Never mutates, never POSTs. A 404 is
  // surfaced distinctly so Screen B can show an honest "not found" state.
  async function loadLiveOpportunity(id) {
    const res = await fetch("/api/review/opportunity/" + encodeURIComponent(id),
  ...
  function historicalBlock(hs) {
    // Only render setups when explicitly available with data. Every other state
    // (not_available, not_implemented, missing, empty) is an honest empty note.
    if (!hs || hs.status !== "available" || !Array.isArray(hs.setups) || !hs.setups.length) {
      return `<div class="empty-note">${esc((hs && hs.note) || "Historical similarity matching isn't available yet for this setup.")}</div>`;
    }
    return `
  ...
  // Minimal header used by Screen B status states (loading / not found / error).
  function detailStateShell(title, bodyHTML) {
    return `
      <header class="detail-head">
        <button class="back-btn" data-goto="brief">‹ Back</button>
        <span class="spacer"></span><span class="detail-title">${esc(title)}</span><span class="spacer"></span>
      </header>
      <div class="content">${bodyHTML}</div>${tabbar("opp")}`;
  }

  function renderDetail() {
    // Live mode: Screen B is driven by GET /api/review/opportunity/{idea_id}.
    // Show honest loading / 404 / error states and never fall back to mock detail.
    if (dataMode === "live") {
      if (state.detailLoading) {
        return detailStateShell("Opportunity",
          `<div class="empty-note" style="margin-top:60px">Loading opportunity…</div>`);
      }
      if (state.detailError === "not_found") {
        return detailStateShell("Opportunity",
          `<div class="empty-note" style="margin-top:60px">Opportunity not found.</div>`);
      }
      if (state.detailError) {
        return detailStateShell("Opportunity",
          `<div class="data-warning" style="margin-top:24px">⚠ ${esc(state.detailError)}</div>
           <div class="empty-note" style="margin-top:18px">Live opportunity detail is unavailable right now.</div>`);
      }
    }

    const d = resolveDetail(state.currentOppId);
    if (!d) return detailStateShell("Opportunity",
      `<div class="empty-note" style="margin-top:60px">Opportunity not found.</div>`);
    const hd = d.header, c = d.conviction, t = d.thesis, ai = d.ai_explanation;

    // Null-safe view fields (live payloads legitimately omit these).
    const chips = Array.isArray(hd.chips) ? hd.chips : [];
    const hasTrend = Array.isArray(c.trend_series) && c.trend_series.length >= 2
      && Array.isArray(c.trend_labels) && c.trend_labels.length === c.trend_series.length;
    const trendArrow = c.trend_direction === "up" ? "↗" : c.trend_direction === "down" ? "↘" : "→";
    const bull = Array.isArray(t.bull_case) ? t.bull_case : [];
    const bear = Array.isArray(t.bear_case) ? t.bear_case : [];
    const evidence = Array.isArray(d.supporting_evidence) ? d.supporting_evidence : [];
    const aiBullets = Array.isArray(ai.bullets) ? ai.bullets : [];
      ...
      <div class="content" style="padding-bottom:24px">
        ...
        <div class="det-id-row">
          ...
          <div class="det-id-meta">
            ...
            <div class="det-chips">${[esc(hd.ticker)].concat(chips.map(esc)).join(" · ")}</div>
            <div class="dir-row">Direction: <span class="badge-dir ${hd.direction === "LONG" ? "long" : "short"}">${esc(hd.direction)} ${hd.direction === "LONG" ? "↑" : "↓"}</span> &nbsp; <span class="chip">${orDash(hd.strategy)}</span></div>
          </div>
          ${gauge(c.score, 78)}
        </div>

        <section class="panel">
          <div class="panel-head"><span class="panel-eyebrow">Conviction Trend (7 Days)</span>${hasTrend && c.trend_text ? `<span class="view-all" style="cursor:default">${esc(c.trend_text)} ${trendArrow}</span>` : ""}</div>
          ${hasTrend
            ? lineChart(c.trend_series, c.trend_labels)
            : `<div class="empty-note">Conviction history isn't available yet for this idea.</div>`}
        </section>

        <section class="panel" style="padding:0">
          <div class="metrics-row">
            <div class="metric"><div class="ml">Expected Move</div><div class="mv up">${orDash(c.expected_move_text)}</div></div>
            <div class="metric"><div class="ml">Risk Level</div><div class="mv" style="color:var(--amber)">${orDash(c.risk_level)}</div></div>
            <div class="metric"><div class="ml">Hold Period</div><div class="mv">${orDash(c.hold_period_text)}</div></div>
            <div class="metric"><div class="ml">Win Prob</div><div class="mv">${c.win_probability != null ? esc(c.win_probability) + "%" : "—"}</div></div>
          </div>
        </section>
        ...
        <section class="panel">
          <div class="bullbear">
            <div class="bb-col bull"><h4>Bull Case</h4><ul class="bb-list bull">${bull.map((x) => `<li>${esc(x)}</li>`).join("")}</ul></div>
            <div class="bb-col bear"><h4>Bear Case</h4><ul class="bb-list bear">${bear.map((x) => `<li>${esc(x)}</li>`).join("")}</ul></div>
          </div>
        </section>` : ""}

        <section class="panel">
          <div class="panel-head"><span class="panel-eyebrow">Why This Matters</span></div>
          <div class="why-body">${t.why_this_matters ? esc(t.why_this_matters) : esc((t.note) || "No thesis text available yet.")}</div>
        </section>
        ...
        <section class="panel">
          ...
          ${evidence.length
            ? `<div class="chips-wrap">${evidence.map((s) => `<span class="chip">${esc(s.label)}</span>`).join("")}</div>`
            : `<div class="empty-note">No supporting evidence recorded yet.</div>`}
        </section>
        ...
        <section class="panel">
          ...
          ${aiBullets.length
            ? `<ul class="ai-list">${aiBullets.map((x) => `<li>${esc(x)}</li>`).join("")}</ul>`
            : `<div class="empty-note">${esc(ai.availability === "insufficient_data" ? "No AI explanation available for this idea yet." : "AI explanation unavailable.")}</div>`}
          ${ai.probability != null ? `<div class="prob-line">Estimated probability of success: <b>${esc(ai.probability)}%</b></div>` : ""}
        </section>

        <section class="panel">
          <div class="panel-head"><span class="panel-eyebrow">Key Risks</span></div>
          ${risks.length
            ? `<ul class="risk-list">${risks.map((x) => `<li>${esc(x)}</li>`).join("")}</ul>`
            : `<div class="empty-note">No key risks recorded yet.</div>`}
        </section>
  ...
  function explainSheet() {
    const d = resolveDetail(state.sheet.oppId);
    // No detail available (e.g. live mode before/without a loaded opportunity).
    // Render an inert, schema-free shell so backdrop handlers exist without
      ...
      return `
        <div class="sheet-backdrop ${state.sheet.open ? "open" : ""}" data-sheet-backdrop>
          <div class="sheet" data-stop>
            ...
            <div class="sheet-scroll"><div class="empty-note" style="margin-top:24px">Detail unavailable.</div></div>
  ...
  function explainTabBody(d, tab) {
    ...
    if (tab === "ai") {
      ...
      return `
        ...
        ${aiBullets.length
          ? `<ul class="ai-list">${aiBullets.map((x) => `<li>${esc(x)}</li>`).join("")}</ul>`
          : `<div class="empty-note">No AI explanation available for this idea yet.</div>`}
        ${ai.probability != null ? `<div class="prob-line">Estimated probability of success: <b>${esc(ai.probability)}%</b></div>` : ""}
  ...
  async function openDetail(id) {
    ...
    state.detailError = "";
    if (dataMode !== "live") {
      // Mock mode: detail comes straight from the mock dataset (synchronous).
    ...
    // Live mode: read-only fetch of GET /api/review/opportunity/{id}. Paint a
    // loading state first, then swap in the validated payload — or an honest
    // not_found / error state. Never falls back to mock detail in live mode.
    state.liveDetail = null;
    state.detailLoading = true;
    render();
    app.scrollTop = 0;
    try {
      const payload = await loadLiveOpportunity(id);
      const problem = validateOpportunity(payload);
      if (problem) throw new Error(problem);
      if (state.screen !== "detail" || Number(state.currentOppId) !== Number(id)) return; // user navigated away
      state.liveDetail = payload;
      state.detailError = "";
    } catch (err) {
      if (state.screen !== "detail" || Number(state.currentOppId) !== Number(id)) return;
      state.liveDetail = null;
      state.detailError = err && err.notFound ? "not_found"
        : "Live opportunity unavailable (" + (err && err.message ? err.message : "fetch failed") + ").";
    } finally {
      if (state.screen === "detail" && Number(state.currentOppId) === Number(id)) {
        state.detailLoading = false;
        render();
      }
    }
