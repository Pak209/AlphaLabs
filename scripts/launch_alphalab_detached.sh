...
if ! /usr/bin/curl -fsS "${DASHBOARD_URL}/api/health" >/dev/null 2>&1; then
  nohup "$HOME/Projects/AlphaLab/.venv/bin/python" -m alpha_lab.main --port 8787 > alpha_lab/data/alphalab.log 2>&1 &
fi
if ! /usr/bin/curl -fsS "${WEBHOOK_URL}/health" >/dev/null 2>&1; then
  nohup "$HOME/Projects/AlphaLab/.venv/bin/python" -m paper_trader.main serve --host 127.0.0.1 --port 8765 --dry-run > paper_trader/logs/webhook.log 2>&1 &
fi
for i in {1..60}; do
  if /usr/bin/curl -fsS "${DASHBOARD_URL}/api/health" >/dev/null 2>&1; then
